...
word: str = "python"
guess: str = str(input(f"What is your {len(word)}-letter guess? "))
playing: bool = True
...
while idx_word < len(word):
    if (guess[idx_word] == word[idx_word]):   # if the letter in the guess is the same as the letter of the secret at the same index
        ...
        while yellow_bool is False and idx_yellow < len(word):   # while the bool is false and yellow index is less than length of secret word
        ...
        if (yellow_bool is True):   # if yellow bool is true at this index
