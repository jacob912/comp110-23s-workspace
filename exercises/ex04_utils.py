...
def all(list_nums: list[int], number: int) -> bool:
    """Seeing if all the numbers in a list are the same as a given number."""
    list_idx: int = 0
    if len(list_nums) > 0:
...
def max(input: list[int]) -> int:
    """Finding the maximum value in a given list."""
    ...
    while input_idx1 < len(input):
        if input[input_idx1] > input[input_idx1 - 1]:  # if the number on the list is bigger than the number before it
            if input[input_idx1] > max_val or max_val == 0:  # if the number is bigger than the current max value
...
def is_equal(list1: list[int], list2: list[int]) -> bool:
    """Seeing if 2 lists are identical to each other."""
    idx_list: int = 0
